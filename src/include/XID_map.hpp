...
class XID_map {
  ...
  public:
		...
		XID_map(const char *str , xercesc_2_2::DOMNode *IncDocRoot);
		void SetRootElement( xercesc_2_2::DOMNode *IncDocRoot );
		...
		std::string String(xercesc_2_2::DOMNode *node, bool printFirstAvailXID=false) ;

		/* Functions that manage XID-2-Node mappings
		 */
		
		void     registerNode( xercesc_2_2::DOMNode *node, XID_t xid );
		void     removeNode( xercesc_2_2::DOMNode *node ) ;
		
		xercesc_2_2::DOMNode* getNodeWithXID(const XID_t xid);
		bool     findNodeWithXID(const XID_t xid);
		XID_t    getXIDbyNode(xercesc_2_2::DOMNode *node);
		...
		void mapSubtree   ( const char *str, xercesc_2_2::DOMNode *node ) ;
		void removeSubtree( xercesc_2_2::DOMNode *node ) ;

		/* PRIVATE */
		
		std::string StringFromList( std::vector<XID_t> xidList, bool writeFirstAvail = false ) ;

		static XID_t getXidFromAttribute(xercesc_2_2::DOMNode* elem, const XMLCh *attrName, bool parenthesis=false);

	private:

		void _internal_TraceTree( std::vector<XID_t> &xidList, xercesc_2_2::DOMNode *node ) ;
		static std::string xidString(XID_t x) ;

		void registerSubtree( XidMap_Parser &parse, xercesc_2_2::DOMNode *node ) ;
		void _internal_registerSubtree( XidMap_Parser &parse, xercesc_2_2::DOMNode *node );
		
		int                    referenceCount ;
		std::map<XID_t, xercesc_2_2::DOMNode*> *nodeByXid ;
		std::map<xercesc_2_2::DOMNode*, XID_t> *xidByNode ;
		XID_t                  firstAvailableXID ;

		xercesc_2_2::DOMNode*               docRoot ;
