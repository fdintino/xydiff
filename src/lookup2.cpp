...
unsigned long hash( unsigned char *key, unsigned long length, unsigned long initval);
	
hash32::hash32 ( void ) {
	value = 0 ;
}

hash32::hash32 ( unsigned char *buffer, unsigned long length ) {
	value = hash( buffer, length, 0 ) ;
}

hash32::hash32 ( unsigned char *buffer, unsigned long length, const hash32 &startingValue ) {
	value = hash( buffer, length, startingValue.value ) ;
}
...
hash32::hash32(const char *str) {
	value = hash((ub1*)str, sizeof(char)*strlen(str), 0);
}

hash32::hash32(const char *str, const hash32 &startingValue) {
	value = hash((ub1*)str, sizeof(char)*strlen(str), startingValue.value);
}
